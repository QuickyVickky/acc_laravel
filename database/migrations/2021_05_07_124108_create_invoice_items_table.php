...
class CreateInvoiceItemsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('invoice_items', function (Blueprint $table) {
            $table->id();
            $table->integer('invoice_id')->default(0)->comment('invoice_id');
            $table->integer('products_or_services_id')->default(0)->comment('sales category only');
            $table->string('description',1000)->nullable()->default(NULL)->comment('optional');
            $table->integer('qty')->default(1)->comment('Required');
